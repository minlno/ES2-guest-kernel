...
static int enable_multi_cpu = 0;
module_param(enable_multi_cpu, int, 0660);

static int my_vec = 60;
module_param(my_vec, int, 0660);
...
setnew:
	...
	if (enable_multi_cpu) {
		...
		is_failed = 0;
		for (i=0; i<NR_CPUS; i++) {
			if (per_cpu(vector_irq, i)[my_vec] != VECTOR_UNUSED) {
				is_failed = 1;
				break;
			}
			per_cpu(vector_irq, i)[my_vec] = desc;
		}
		if (is_failed) {
			pr_info("my_vec is already used by cpu %d\n", i);
			for (i=i-1; i>=0; i--) {
				per_cpu(vector_irq, i)[my_vec] = VECTOR_UNUSED;
			}
		}
	}
}
